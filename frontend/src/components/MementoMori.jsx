...
import PhilosophyPanel from './PhilosophyPanel.jsx';
...
function pickQuote(excludeIndex) {
  ...
  } while (QUOTES.length > 1 && idx === excludeIndex);
  return idx;
}
...
export default function MementoMori({ heatmapData, birthday }) {
  ...
  // Random quote index (stable per mount; shuffled on demand)
  const [quoteIdx, setQuoteIdx] = useState(() => pickQuote(-1));
  const quote = QUOTES[quoteIdx];
  const shuffleQuote = useCallback(() => {
    setQuoteIdx((cur) => pickQuote(cur));
  }, []);
  ...
  return (
    <>
      {/* PHILOSOPHY: collapsible "What is Memento Mori?" explainer */}
      <PhilosophyPanel />
      ...
      <div className="card memento-top">
        ...
        <div className="memento-quote-block">
          <p className="memento-quote">
            <span className="memento-quote-text">&ldquo;{quote.text}&rdquo;</span>
            <span className="memento-quote-attrib">
              &mdash; {quote.author}
              <span className="memento-quote-source"> · {quote.source}</span>
            </span>
          </p>
          <button className="memento-quote-shuffle" onClick={shuffleQuote} title="New quote">
            &#8635; New quote
          </button>
        </div>
